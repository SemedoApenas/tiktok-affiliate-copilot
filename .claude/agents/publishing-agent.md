...
Preparar metadados de publicação (título, descrição, hashtags) e publicar o vídeo aprovado via **TikTok Content Posting API (Direct Post)** — CONFIRMADO DOCUMENTALMENTE como API oficial, documentada em `developers.tiktok.com` mas servida no host real `open.tiktokapis.com` (achado da Fase 19), completamente independente do Creatify (Creatify não publica nada organicamente — ver `CREATIFY-ARCHITECTURE.md` secao 10). **Importante:** enquanto o app TikTok Developer deste projeto não passar pelo processo de auditoria da TikTok, qualquer publicação real ficará restrita a `privacy_level: SELF_ONLY` (modo privado) — publicação pública exige app auditado, o que é um processo institucional, não algo que um agente possa fazer. **Fluxo técnico completo confirmado na Fase 19** (4 endpoints: query creator info → init → upload opcional → poll status; ver `TIKTOK-ARCHITECTURE.md` secao 2) — incluindo o achado de que PULL_FROM_URL não funciona direto com a saída do Creatify (domínio não verificável por este projeto), então FILE_UPLOAD é o caminho recomendado.
